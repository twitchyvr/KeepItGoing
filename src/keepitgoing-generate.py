...
DIRECTIVES = {
    ...
    "self_rescue": [
        ...
        'STUCK on a visual bug? `kig ask "why does the tab bar show route path?" --shot-sim` — Opus reads a fresh simulator screenshot AND answers. Or `kig unstuck --shot-sim` for a broader directive.',
        '`kig delegate "task"` creates a git worktree at /tmp/kig-delegate/*, runs Opus scoped to that worktree, returns the diff. Never touches your working tree. Cherry-pick if the diff looks good. Budget: 2h/day default, separate from consult budget.',
        "TOOL CHECK before the 3rd wheel-spin: `mcp__MiniMax__understand_image` (screenshots), `mcp__MiniMax__web_search` (current info), `mcp__context7` (library docs), ios-simulator skill, visual-verify skill, `Agent` tool for subagent dispatch. USE them before escalating.",
        "`kig delegate` requires a NARROW task string. 'Fix the app' is wrong. 'Fix the Jest moduleNameMapper conflict for dynamic imports in __tests__/api/foo.test.ts' is right. Narrow scope = reviewable diff.",
        "wrong-path detection: if you've been investigating the same sub-issue for 10+ minutes, the premise is probably wrong. `kig ask \"am I investigating the wrong thing? Here's what I believe: X. Is X correct?\"`",
        "before filing 'pre-existing issue' or giving up — `kig unstuck`. Opus sees your full session context and will either redirect you or confirm it's actually blocked. Costs seconds, saves hours.",
        "don't confuse 'I tried a thing' with 'I solved it.' If the root cause is still unknown after your fix, the fix is speculative. `kig ask \"did my fix address root cause or just mask a symptom?\"`",
        'if you\'re going to delegate — `kig delegate --dry-run "task"` first to confirm the worktree path + budget impact. Then run without --dry-run to actually execute.',
    ],
    "compaction_amnesia": [
